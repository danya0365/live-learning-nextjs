...
import { AdminChatViewModel } from "@/src/presentation/presenters/admin-chat/AdminChatPresenter";
import { useAdminChatPresenter } from "@/src/presentation/presenters/admin-chat/useAdminChatPresenter";

interface AdminChatListViewProps {
  activeTab: string;
  initialViewModel: AdminChatViewModel;
}

/**
 * AdminChatListView - UI component for the admin chat list dashboard.
 * Following Clean Architecture - this is in the Presentation layer.
 */
export function AdminChatListView({ activeTab, initialViewModel }: AdminChatListViewProps) {
  const { viewModel, loading, error } = useAdminChatPresenter(activeTab, initialViewModel);

  // Fallback to initial during loading if needed, though useAdminChatPresenter handles it
  const displayViewModel = viewModel || initialViewModel;
  const { sessions, counts } = displayViewModel;
    ...
    <div className="min-h-screen bg-background p-4 md:p-8 transition-colors duration-500">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid gap-6">
          {loading && !viewModel ? (
             <div className="glass rounded-[3rem] p-24 text-center animate-pulse">
                <p className="text-text-muted font-bold">กำลังโหลดข้อมูล...</p>
             </div>
          ) : sessions.length === 0 ? (
